...
basemap = get_map(location = unname(st_bbox(traveltime60)),source = "google")

ggmap(basemap)+
   geom_sf(data=traveltime60,inherit.aes = FALSE, fill="SteelBlue", alpha=0.5, color=F)+
  geom_sf(data=traveltime30,inherit.aes = FALSE, fill="DarkBlue",alpha=0.5,color=F)+
  labs(title="Traveltime-Map",
       subtitle="how far can you go by public transport in 30 / 60 min?")+
  theme_void()
